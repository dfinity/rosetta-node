...
use ic_crypto_internal_threshold_sig_bls12381::api::ni_dkg_errors::{
    CspDkgCreateDealingError, CspDkgCreateFsKeyError, CspDkgLoadPrivateKeyError,
        ...
        match algorithm_id {
            AlgorithmId::NiDkg_Groth20_Bls12_381 => {
                let maybe_resharing_secret_key_bytes = match maybe_resharing_secret_key {
                    Some(secret_key) => {
                        let secret_key_bytes = specialise::groth20::threshold_secret_key(secret_key).map_err(
            ...
            other => Err(CspDkgCreateDealingError::from(
                ni_dkg_errors::CspDkgCreateReshareDealingError::UnsupportedAlgorithmId(other),
            )),
        }
    }
