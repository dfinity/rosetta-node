...
pub fn initial_dkg_transcript_for_nodes_in_subnet(
    subnet_id: SubnetId,
    nodes: &[NodeId],
    ...
    let keys_collected_from_nodes = encryption_public_keys(
...
pub fn load_transcript_for_each(
    nodes: &[NodeId],
    ...
    nodes
        .iter()
        .for_each(|node_id| load_transcript(transcript, crypto_components, *node_id));
}
...
pub fn load_transcript(
    ...
    if let Err(e) = DkgAlgorithm::load_transcript(
        crypto_for(node_id, crypto_components).deref(),
        transcript,
        node_id,
    ) {
        panic!("{:?}", e);
