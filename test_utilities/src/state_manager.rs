...
impl StateManager for FakeStateManager {
    ...
    fn get_state_hash_at(&self, height: Height) -> StateManagerResult<Option<CryptoHashOfState>> {
        ...
        states
            ...
            .ok_or_else(|| StateManagerError::StateRemoved(height))
    }
...
impl CertifiedStreamStore for FakeStateManager {
    ...
    fn encode_certified_stream_slice(
        ...
        let begin_index = msg_begin.unwrap_or_else(|| stream.messages.begin());
        if begin_index < stream.messages.begin() || stream.messages.end() < begin_index {
