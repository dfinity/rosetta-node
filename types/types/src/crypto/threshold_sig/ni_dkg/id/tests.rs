...
fn should_convert_ni_dkg_id_to_proto() {
    ...
    let target_id = [42; NI_DKG_TARGET_ID_SIZE];
    let height = 7;
    let id = NiDkgId {
        start_block_height: Height::new(7),
        dealer_subnet: SubnetId::from(principal_id),
        dkg_tag: NiDkgTag::HighThreshold,
        target_subnet: NiDkgTargetSubnet::Remote(NiDkgTargetId::from(target_id)),
    };
        ...
        assert_eq!(
            ...
            NiDkgId {
                ...
                target_subnet: match val {
                    ...
                    Some(_) => NiDkgTargetSubnet::Remote(NiDkgTargetId::from(target_id)),
                },
            }
        );
    }
...
fn should_return_error_if_remote_target_id_invalid_when_parsing_proto() {
    let target_id_size = NI_DKG_TARGET_ID_SIZE - 2;
    let proto = NiDkgIdProto {
    ...
    assert_eq!(
        result.unwrap_err(),
        NiDkgIdFromProtoError::InvalidRemoteTargetIdSize
    );
}
...
fn should_return_error_if_ni_dkg_tag_invalid_when_parsing_proto() {
    ...
    let proto = NiDkgIdProto {
        start_block_height: 7,
        dealer_subnet: vec![42; PrincipalId::MAX_LENGTH_IN_BYTES],
        remote_target_id: Some(vec![42; NI_DKG_TARGET_ID_SIZE]),
