...
use ic_registry_common::{
    bootstrap_registry_data_provider::INITIAL_REGISTRY_VERSION,
...
use ic_registry_keys::{
    ...
    ROOT_SUBNET_ID_KEY, SUBNET_LIST_KEY,
};
...
use crate::{
    ...
    node::{InitializeNodeError, NodeConfiguration, NodeIndex},
...
pub type InitializedTopology = BTreeMap<u64, InitializedSubnet>;
...
impl IcConfig {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        Self {
            ...
            initial_replica_download_url: replica_download_url
                .unwrap_or_else(|| Url::parse("http://example.dfinity.org").unwrap()),
    ...
    pub fn initialize(mut self) -> Result<InitializedIc, InitializeError> {
        ...
        let replica_version_record = ReplicaVersionRecord {
            ...
            sha256_hex: self.initial_replica_hash.unwrap_or_else(|| "".into()),
            node_manager_binary_url: self
                .initial_nodemanager_url
                .map(|url| url.to_string())
                .unwrap_or_else(|| "".into()),
            node_manager_sha256_hex: self
                .initial_nodemanager_sha256_hex
                .unwrap_or_else(|| "".into()),
        ...
        let (mut updates, _) = data_provider.get_updates_since(ZERO_REGISTRY_VERSION)?;
