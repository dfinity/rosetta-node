...
use openssl::pkey;
use std::{env, fs::File, io, io::Read, path::PathBuf, str::FromStr, string::ToString};
...
pub fn ensure_file_exists(file: &PathBuf) -> io::Result<()> {
    if !file.is_file() {
...
pub fn ensure_file_extension(file: &PathBuf, ext: &str) -> io::Result<()> {
