...
fn can_create_canister_from_another_canister() {
    utils::simple_canister_test(|canister| {
        ...
        assert_eq!(
            canister.update(wasm().call_with_cycles(
                ic00::IC_00,
    ...
    utils::simple_canister_test(|canister| {
        ...
        assert_matches!(
            canister.update(wasm().call_simple(
                ...
                call_args().other_side(canister_id_record.clone())
            )),
        ...
        assert_matches!(
            canister.update(wasm().call_simple(
                ic00::IC_00,
                Method::CanisterStatus,
                call_args().other_side(canister_id_record)
            )),
            Ok(WasmResult::Reply(res)) if CanisterStatusResultV2::decode(&res).unwrap().status() == CanisterStatusType::Running
        );
    });
        ...
        assert_matches!(
            ...
            Ok(WasmResult::Reply(res)) => assert_canister_status_result_equals(
                CanisterStatusResultV2::new(
                    CanisterStatusType::Running,
                    Some(ic_crypto_sha256::Sha256::hash(UNIVERSAL_CANISTER_WASM).to_vec()),
                    canister_a.get(),
