...
fn test_query_trap_recovery() {
    ...
    utils::canister_test(move |test| {
        ...
        assert!(test.query_raw(canister_id, "trap", vec![]).is_err());
...
fn test_update_available_memory_3() {
    ...
    utils::canister_test(move |test| {
        ...
        assert!(
            err.description().contains("heap out of bounds"),
            err.to_string()
        );
...
#[test]
/// User sends a query message containing a call to the exec system call. This
/// is not allowed.
fn canister_query_exec_system_call() {
    utils::canister_test(|test| {
        let wat = r#"
            (module
                (import "ic0" "exec"
                    (func $exec (param i32) (param i32) (param i32) (param i32))
                )
                (func $call_exec (export "canister_query call_exec")
                    (call $exec (i32.const 0) (i32.const 8) (i32.const 0) (i32.const 0))
                )
                (memory (export "memory") 10)
                ;; minimal empty wasm: (module)
                (data (i32.const 0) "\00\61\73\6d\01\00\00\00")
            )"#;
        let (canister, _) = test.create_and_install_canister(wat, vec![]);
        let result = test.query(canister, "call_exec", vec![]);
        match result {
            Err(err) => {
                assert_eq!(
                    err.description(),
                    "Calling exec is only allowed in the update call"
                );
                assert_eq!(err.code(), ErrorCode::CanisterContractViolation);
            }
            Ok(x) => panic!("Expected an error but got a result {:?}", x),
        }
    });
}
